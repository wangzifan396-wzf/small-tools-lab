...
const headers = (type) => ({ 'content-type': type, 'x-content-type-options': 'nosniff', 'referrer-policy': 'no-referrer' });
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (child.startsWith('..') || isAbsolute(child)) { res.writeHead(403, headers('text/plain; charset=utf-8')).end('forbidden'); return; }
...
server.listen(PORT, () => console.log(`colorconv playground -> http://localhost:${PORT}/`));
