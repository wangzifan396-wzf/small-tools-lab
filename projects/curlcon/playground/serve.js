...
const server = http.createServer(async (request, response) => {
  ...
  try {
    ...
    if (decodeURIComponent(rawPath).split(/[\\/]/u).includes('..')) { response.writeHead(403, headers('text/plain; charset=utf-8')).end('forbidden'); return; }
    const url = new URL(request.url, `http://localhost:${PORT}`); const pathname = decodeURIComponent(url.pathname || '/');
    const target = resolve(ROOT, `.${pathname === '/' ? '/index.html' : pathname}`); const child = relative(ROOT, target);
    if (child.startsWith('..') || isAbsolute(child)) { response.writeHead(403, headers('text/plain; charset=utf-8')).end('forbidden'); return; }
    const info = await stat(target); const file = info.isDirectory() ? join(target, 'index.html') : target; const body = await readFile(file);
    response.writeHead(200, headers(TYPES[extname(file).toLowerCase()] || 'application/octet-stream')); response.end(request.method === 'GET' ? body : undefined);
  } catch (error) {
...
server.listen(PORT, () => console.log(`curlcon playground -> http://localhost:${PORT}/`));
