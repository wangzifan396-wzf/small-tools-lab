...
test("catalog covers all supported categories", () => {
  ...
  assert.equal(TOOLS.filter((tool) => tool.category === "browser").length, 95);
...
test("search and category filtering are deterministic", () => {
  ...
  assert.deepEqual(TOOLS.filter((tool) => matchesTool(tool, "jwks", "browser")).map((tool) => tool.id), ["jwt-trust"]);
