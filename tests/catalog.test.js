...
test("catalog covers all supported categories", () => {
  ...
  assert.equal(TOOLS.filter((tool) => tool.category === "browser").length, 5);
  assert.equal(TOOLS.filter((tool) => tool.category === "cli").length, 6);
...
test("search and category filtering are deterministic", () => {
  ...
  assert.equal(TOOLS.filter((tool) => matchesTool(tool, "", "cli")).length, 6);
});
